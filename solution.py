...
def f1_check_minimum(B, a, b):
    ...
    dfdx2 = (B+B.T).T - 2 * np.identity(2)
    check = (np.linalg.det(dfdx2) > 0 and dfdx2[0, 0] > 0)
    return check
...
def f3_grad_exact(x):
    ...
    # FX = 1/10 * gX
    # gX = np.linalg.det(fX)
    # fX = 1/100 * np.identity(2) + x @ x.T

    # FxdX = 1/10 * 1/(gX)
    # gXdX = np.linalg.det(fX) * np.linalg.tr(np.linalg.inv(fX) @ fXdX)
    # fXdX =  
    # Method 1: Directly use the formular in MML book
    X = (1/100.) * np.identity(2) + x @ x.T
    x_xT_dx = np.array([[ [2*x1, x2 ],
                         [x2  ,  0 ] ],
                       [ [0   , x1 ],
                         [x1  ,2*x2] ]])
    grad_third = 1/10 * np.array([np.trace(np.linalg.inv(X) @ x_xT_dx[0]), np.trace(np.linalg.inv(X) @ x_xT_dx[1])])

    # Method 2: Solve det function using det(A) = ad - bc (for 2x2 only). Then get derivative
...
if __name__ == '__main__':
    ...
    f1_grad_fd(dummy_input)
    f2_grad_fd(dummy_input)
    f3_grad_fd(dummy_input)
    f1_grad_exact(dummy_input)
    f2_grad_exact(dummy_input)
    f3_grad_exact(dummy_input)
    # gradient_descent(fn, grad_fn, start_x=-0.5, start_y=0.5, lr=0.05, n_steps=50)
    # print(check)
